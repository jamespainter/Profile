...
VALUE& arrayHashIterator<KEY, VALUE>::operator*() const{
	...
	//VALUE *temp = valueArray[this->index];
...
arrayHashIterator<KEY, VALUE> arrayHashIterator<KEY, VALUE>::operator++() {
	...
	++this->index;
	while (this->index != arraySize)
	{
		if (statusArray[this->index] == 1)
		{
...
void arrayHashTable<KEY, VALUE>::add(const KEY& key, const VALUE& value)  {
	...
	// 0: we can use it 
	// 1: keep looking 
	int indexKey = hash(key, arraySize);
	int holdKey = indexKey;
	while (indexKey <= arraySize)
	{

		if (indexKey == arraySize)
		{

			//indexKey = (indexKey + 1) % arraySize;
			indexKey = 0;
		}
		if (statusArray[indexKey] == 0 || statusArray[indexKey] == -1) {
			valueArray[indexKey] = value;
			keyArray[indexKey] = key;
			statusArray[indexKey] = 1;
			break; 
		}
		else 
		{// ++statusArray[indexKey];
		++indexKey;
		if (holdKey == indexKey)
		{
			break;
...
VALUE arrayHashTable<KEY, VALUE>::item(const KEY& key) {
	...
	// do while (index == arraysize) or while set it to zero
	// check if statusArray 
	// if its zero throw error(); 
	// if its -1, keep looking
	// if its 1, check key arry
	// if it doesn't match keep looking found, return valueArray at that index. 
	int indexKey = hash(key, arraySize);
	int holdKey = indexKey; 
	while (indexKey < arraySize)
	{
		...
		else if (statusArray[indexKey] == 1)
		{
			if (keyArray[indexKey] == key)
			{
				return valueArray[indexKey];
			}
		}
		else if (statusArray[indexKey] == 0)
		{
			throw Error();
		}
		++indexKey;
...
void arrayHashTable<KEY, VALUE>::remove(const KEY& key)  {
	...
	while (indexKey <= arraySize)
	{
		if (keyArray[indexKey] == key && statusArray[indexKey] != -1)
		{
			keyArray[indexKey] = -1;
			statusArray[indexKey] = -1;
