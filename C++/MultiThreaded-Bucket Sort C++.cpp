...
// Modify add int threadID into placeIntoBuckets 
// start and end index 
// local buckets object  
// loop to ending index 
void placeIntoBuckets() {
	 
	int count = 0;
	
	int bucketSize = listSize / numBuckets; 
	int startVal = 0; 
	int i = 0; 
	...
	while (count < numBuckets)
	{
		...
		for (i = startVal; i < bucketSize+startVal; ++i)
		{
			globalBuckets->addItem(count, list[i]);
...
void sortEachBucket(int threadID) {
	...
	int startingIndex = 0;
	int endingIndex =  listSize / numBuckets; 
	/*int localCount = 0;
	...
	recQuickSort(globalBuckets->getBucketArray(threadID), startingIndex,endingIndex);
	//recQuickSort(list, 0, listSize);
...
void combineBuckets() {
	//int count = 0; 
	int listSizeOfNumBuckets = listSize / numBuckets;
	int startVal = 0;
	////TODO: Copy each bucket back out to the original list[] array
	for (int i = 0; i < numBuckets; i++)
	{
		globalBuckets->copyBucketToAnotherArray(i, list, startVal);
		//startVal = startVal + listSizeOfNumBuckets;
		
	}
	recQuickSort(list, startVal, listSize);
		
		
		
	////listSizeOfNumBuckets = listSizeOfNumBuckets + (listSize / numBuckets); 
			//count++;
...
void bucketSort() {
	...
	// call sortEachBucket in parallel, pass in your i for your loop
	for (int i = 0; i < numBuckets; i++){
