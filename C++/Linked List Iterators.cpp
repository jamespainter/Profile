...
Iterator<T>& Iterator<T>::operator--() {
	...
	if (current->backward != NULL && offTheRightEdge == false)
	{
...
void ourReverse(T& begin, T& end) {
	...
	/*while (begin.offTheRightEdge == false)
	{
		swap(begin.current->info, end.current->info);
		begin++; 
		end--; 
	...
	//{
	//
	//	//swap(*beginTemp.current, *endTemp.current);
	//	endTemp--;
	//}
}
//----------------------------------------------------------------------------------------------------------------------------------------
