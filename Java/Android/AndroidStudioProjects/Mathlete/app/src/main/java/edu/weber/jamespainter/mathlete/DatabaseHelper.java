...
/**
 * Created by JamesPainter on 4/3/2017.
 */
...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public long insertUser(String user_name, String password, String startDate, String endDate)
    {
        long rowID = -1;

        ContentValues newCourse = new ContentValues();
//        newCourse.put("ID", user_ID);
        newCourse.put("Name", user_name);
        newCourse.put("Code", password);
...
//       db.execSQL("delete from courses");
    ...
    public boolean checkUser(String username, String password)
    {
        ...
        {
            Log.d("text", "Querying one Course");
            cursor = database.rawQuery("SELECT * FROM USERS WHERE username = ? AND password = ?", params);
        }
        ///Check to see if user count comes back
...
//        db.execSQL(createQuery);
        ...
        String createQuery = "CREATE TABLE users " +
                "(_id integer primary key autoincrement," +
                "ID TEXT,username TEXT, password TEXT, StartDate TEXT, EndDate TEXT);";
